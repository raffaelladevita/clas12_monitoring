...
public class monitor2p2GeV {
	...
	public void fillTraj(DataBank bank){
		for(int iI=0;iI<bank.rows();iI++){
			if(bank.getShort("detector",iI)<3 && bank.getShort("trkId",iI)==e_track_ind){
				found_e_FMM = 1;
				float px = bank.getFloat("px", iI);
				float py = bank.getFloat("py", iI);
				float pz = bank.getFloat("pz", iI);
				e_FMMmom[bank.getShort("detector",iI)+1] = (float)Math.sqrt(px*px+py*py+pz*pz);
				e_FMMtheta[bank.getShort("detector",iI)+1] = (float)Math.toDegrees(Math.acos(pz/e_FMMmom[bank.getShort("detector",iI)+1]));
				e_FMMphi[bank.getShort("detector",iI)+1] = (float)Math.toDegrees(Math.atan2(py,px));
				e_FMMvz[bank.getShort("detector",iI)+1] = bank.getFloat("z",iI);
			}
		}
	}
	...
	public void fillTraj_HTCC(DataBank trajBank, DataBank htcc){
		for(int r=0;r<trajBank.rows();r++){
			if(trajBank.getShort("pindex",r)==e_part_ind){
				found_eTraj=1;
				if(trajBank.getShort("detector",r) == 0) {
